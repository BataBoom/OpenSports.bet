...
*/
include_once('dashboard/nhlOdds.php');
...
$button0 = array(
   'checkboxWrapper'     => '<div class="round-checkbox"></div>',
   'inlineCheckboxLabelClass' => 'styled-checkbox'
...
$form[$i]->addCheckbox($MLMenu[$i], 'Moneyline?', 1, 'class=is-small is-red' , checked);
$form[$i]->addCheckbox($OUMenu[$i], 'Totals?', 2, 'class=is-small is-red' , unchecked);
$form[$i]->addCheckbox($RLMenu[$i], 'Spreads?', 3, 'class=is-small is-red' , checked);
...
$form[$i]->addInput('hidden', 'MLDate', $newDBDate[$i][0]);
...
$form[$i]->addInput('hidden', 'OUDate', $newDBDate[$i][0]);
...
$form[$i]->addInput('hidden', 'RLDate', $newDBDate[$i][0]);
...
$form[$i]->addInput('hidden', 'start', $newDBDate[$i][0]);
$form[$i]->addInput('hidden', 'niceStart', $newtime[$i][0]);



$form[$i]->addHtml('<footer class="card-footer">', $Menu[$i], 'before');
$form[$i]->addBtn('submit', 'NHLe', 1, 'Place Wager <i class="fas fa-arrow-right fa-fw"></i>', $attr = 'class=card-footer-item, id=ok', $Menu[$i]);
...
$form[$i]->addHtml('</footer>', $Menu[$i], 'after');
...
.card-header {
  background-color:rgba(49, 189, 196, 0.44);
...
.card-content {
    ...
    background-image: linear-gradient(15deg, #13547a 0%, #3234aa 100%);
  position:flex;
  width:100%;
  padding: 2px 16px;
}
...
<div id="posts-feed" class="container" data-open-sidebar data-page-title="NHL Odds">
  
  <div class="block"></div>
 <div class="cover-bg">
    <img class="cover-image" src="assets/img/hockey.jpg" data-demo-src="assets/img/hockey.jpg" alt="">
    <div class="avatar">
...
<img src="assets/logos/nhl/<?php echo $homeImages[$i].'.png'; ?>">
  <div class='block'></div>
 </figure><figure class="image is-96x96">
  <img src="assets/logos/nhl/<?php echo $awayImages[$i].'.png'; ?>">
    <div class='block'></div>
</figure>

  <p class="card-header-title" style="color: #fff;"> <?php
 echo $mlTeam[$i][0]  . '  VS   ' . $mlTeam[$i][1];?> </p>
<p class="subtitle is-6" style="color: #fff; opacity: 0.5;">
