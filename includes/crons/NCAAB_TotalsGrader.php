...
$fetchBets = readW("league", "ncaab", "bets", $limit);
$readBets = array_values(array_filter($fetchBets, function($var) {
    return ($var['type'] == 'Totals');
...
for ($i = 0; $i < $ender; ++$i){
    if (str_starts_with($opt[$i], 'Over')){
        ...
        $cleanOvers = filter_var($opt[$i], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);  
        $Overs[] = array('betSlip'=>$readBets[$i], 'count'=>$cleanOvers, 'whutAmi'=> "Over");

    }elseif (str_starts_with($opt[$i], 'Under')){
       
         $cleanUnders = filter_var($opt[$i]['option'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);  
